...
class Processor(ABC):
    ...
    def _process_batch(self, batch: list[tuple[datetime, pd.DataFrame, timedelta]]):
        results = []
        for ts, df, dt in batch:
            # TODO: multiprocessing
            procd = _picklable_worker(self._worker, df, timestamp=ts, interval=dt)
            procd |= {"timestamp" : ts}
            results.append(procd)
